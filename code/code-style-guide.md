...
no titles or subtitles for any plots

only color palettes allowed are brewer or viridis, no hardcoding any colors

never use cat() or print()

for PCA use these packages: library(FactoMineR); library(factoextra)
